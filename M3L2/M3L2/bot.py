...
!project_info  - belirli bir projeye ait tüm bilgileri görüntülemek
...
async def project_info(ctx):
    ...
    if not projects:
        await ctx.send(
            'Henüz herhangi bir projeniz yok!\n'
            'Bir tane eklemek için !new_project komutunu kullanabilirsiniz.'
        )
        return

    project_names = [x[2] for x in projects]

    await ctx.send("Bilgilerini görmek istediğiniz projeyi seçin:")
    await ctx.send("\n".join(project_names))
    ...
    if project_name.content not in project_names:
        await ctx.send("Bu projeye sahip değilsiniz veya böyle bir proje bulunamadı!")
        return

    selected_project = next(
        x for x in projects if x[2] == project_name.content
    )

    await ctx.send(
        f"📁 **Proje Bilgileri**\n\n"
        f"**Proje adı:** {selected_project[2]}\n"
        f"**Bağlantı:** {selected_project[4]}"
    )
