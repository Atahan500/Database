...
class DB_Manager:
    ...
    def add_black_panter_column(self):
        conn = sqlite3.connect(self.database)
        cursor = conn.cursor()

        # Projects tablosundaki mevcut sütunları kontrol et
        cursor.execute("PRAGMA table_info(projects)")
        columns = [column[1] for column in cursor.fetchall()]

        # black_panter yoksa ekle
        if 'black_panter' not in columns:
            cursor.execute("""
                ALTER TABLE projects
                ADD COLUMN black_panter TEXT
            """)
    ...
    def get_project_info(self, user_id, project_name):
        sql = """
        SELECT project_name, description, url, status_name, black_panter
        FROM projects
        JOIN status
...
if __name__ == '__main__':
    ...
    # black_panter sütununu ekle
    manager.add_black_panter_column()
